...
public class MeasureTypeService {
  ...
  public Flux<RestrictiveMeasure> getSignpostingMeasureTypeContents(
      List<Measure> measures, String commodityCode, final UkCountry destinationUkCountry) {
    return Flux.fromIterable(measures)
        .flatMap(
            measure -> {
              var locale = AppConfig.LOCALE;
              return Mono.zip(
                  this.measureTypeDescriptionContentRepo
                  ...
                  this.measureOptionService
                      .getMeasureOptions(measure.getMeasureConditions(), destinationUkCountry)
                      .collectList(),
                  Mono.just(measure));
            })
        .filter(
        ...
        .map(
            measureTypeDescriptionWithOptions -> {
              if (commoditySpecificMeasureOverrideText.containsKey(commodityCode)
                ...
                String descriptionOverlay = measureTypeDescriptionWithOptions.getT1().getDescriptionOverlay()
